...
export default class AppController {
  constructor(appView, appModel) {
    ...
    this.controlsController = new ControlsController(this.view.getV('controls'), this.bgController.updateBg, this.search.bind(this), this.placeDetailsController.changeTemperatureFar);

    this.search();
  }

  async search(query = '', units) {
    const locationData = await location.request(query);

    if (locationData) {
      this.placeLocationController.updateLocation(locationData.geometry);
      await this.placeDetailsController.updatePlace(locationData, units);
